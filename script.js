...
const workDetails = {
  docval: { name: "DocVal", ask: "Tell me more about DocVal", html: `
    ...
    <p>The honest part: real scans are genuinely hard, and 0.42 is published right next to the pretty synthetic number. Cost is tracked beside accuracy — about $0.005 per document — and CI regression gates re-run the eval on every push. The eval work also surfaced a commercial benchmark whose scoring API was broken server-side; I reported it upstream rather than quietly dropping the comparison.</p>
    <p class="card-links"><a href="https://docval-yy4s.onrender.com" target="_blank" rel="noopener">Live demo ↗</a> · <a href="https://github.com/Sero01/docval" target="_blank" rel="noopener">GitHub ↗</a></p>`},
  ...
  council: { name: "Council", ask: "Tell me more about Council", html: `
    <p class="lead">Council is a terminal chat where Claude Code and Codex work the same repo alongside me — and the Inbox Firewall is the study they built in it.</p>
    <p>Both agents run through my own logged-in CLIs, see each other's replies, and can answer one another for a capped number of hops before the floor returns to me. Files and commands are theirs to touch. It works because the work is split by ownership: one agent owned the action DSL, the corpus, the harness and the baselines; the other owned the model adapter, confidence, calibration and tests — and neither edits the other's files without a handoff in the chat.</p>
    <p>What they built in it answers a real question: <b>how small can a model get before local automation stops being safe?</b> A ~26M router turns each inbox message into one line of a closed, typed action DSL. A hand-written validator parses it — never <code>eval</code> — a tier policy decides what may auto-execute, and anything ambiguous, hostile, or malformed escalates instead.</p>
    <div class="stat-grid"><div class="stat"><b>1028</b><small>labelled messages, splits disjoint</small></div><div class="stat"><b>88.1%</b><small>baseline action accuracy</small></div><div class="stat"><b>11</b><small>unsafe executions calibration didn't fix</small></div><div class="stat"><b>7</b><small>unsafe after the content-trust layer</small></div></div>
    <p>The headline is the one I didn't expect. The keyword baseline is strong — 88.1% action accuracy at 74.8% coverage — but it confidently acts on 11 messages it should have escalated, and every one of them is a <em>valid</em> action the validator is powerless against. Held-out calibration then drove ECE to 0.000 and moved that count by exactly zero: confidence carries no signal about an adversary it never saw at calibration time. What did move it was a deterministic content-trust layer that treats the message body as untrusted — unsafe 11 → 7, for about two points of coverage. <b>Calibration is a coverage instrument, not a safety one.</b></p>
    <p>Reported plainly, because it's the honest part: the remaining 7 are phishing and social engineering — a different threat needing a different layer, and unsafe → 0 is genuinely hard. The 26M fine-tune is deferred, not done. The training exporter is built and verified, but this machine has no training runtime, and I won't quote a number I haven't measured. Both are public and runnable locally; there's no hosted demo yet.</p>
    <p class="card-links"><a href="https://github.com/Sero01/tiny-router-poc" target="_blank" rel="noopener">Study on GitHub ↗</a> · <a href="https://github.com/Sero01/council" target="_blank" rel="noopener">Council on GitHub ↗</a></p>`},
  evals: { name: "the eval harness", ask: "Tell me more about the eval harness", html: `
    ...
    <p>DocVal and ReconMatch sit on the same evaluation spine — schema validation, then arithmetic checks, then a held-out benchmark the models never train or prompt against. Cost is a first-class metric next to accuracy, and CI refuses a push that regresses either.</p>
    <p>What the harness caught that eyeballing never would: silent output truncation on long statements, nondeterminism at temperature zero, a provider injecting errors mid-stream, and a third-party scorer that was broken on their side. Benchmarks lie; a good harness is how you catch them doing it.</p>
    <p class="card-links"><a href="https://github.com/Sero01/docval" target="_blank" rel="noopener">Read the harness code ↗</a></p>`}
};
...
const content = {
  ...
  work: { title: "Selected work", thread: [
    ...
    <div class="work-list">
      ...
      <button class="work-card" data-detail="council"><span class="work-head"><h3>Council</h3><b>2 agents, 1 repo</b></span><p>Claude Code + Codex co-building a tiny-model safety study in my terminal.</p><span class="work-more">Click to know more →</span></button>
